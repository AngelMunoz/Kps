...
```fsharp
type StateChange = {
  entityUpdates: HashMap<int<EntityId>, All>
  events: GameEvent IndexedList
}
...
- [x] Create `IAbilityStore` and `IEffectStore` interfaces
- [x] Create `EngineServices` type for dependency injection, including the `rng` callback.
- [x] Create `StateChange` type for immutable updates

### Phase 2: Resolution.fs Purification

- [x] Update `ResolverParams` to include `EngineServices`
- [x] Refactor `checkTauntTarget` to use injected effect store
- [x] Refactor `validateAction` to use injected ability store
- [x] Update `processEffect` to use injected effect store
- [ ] Refactor combat functions to use the RNG from `EngineServices`.
- [x] Refactor `step` function to return `StateChange` instead of mutating
