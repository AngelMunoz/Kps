...
type ``Scenario Transition Tests``() =
  ...
  member _.``Transition detection works for nearby entities``() =
    ...
    | ValueNone ->
      Assert.True(false, "Expected to find transition trigger")
  ...
  member _.``Entity migration works correctly``() =
    ...
    let ((targetId, updatedEntity), (sourceId, entityToRemove)) =
      TransitionExecution.migrateEntity entityId entity newPosition fromScenarioId toScenarioId
  ...
  member _.``Visual transition effects work correctly``() =
    ...
    let completedEffect = VisualTransitionEffects.updateEffect 1.0f halfwayEffect
  ...
  member _.``Connected scenarios are properly linked``() =
    let (town, wilderness, dungeon) = ScenarioDefinitions.createConnectedScenarios()
  ...
  member _.``Town scenario has correct layout``() =
    ...
    let blockedObjects = terrainArray |> Array.filter (fun obj -> obj.TerrainType = Blocked)
    let waterObjects = terrainArray |> Array.filter (fun obj -> obj.TerrainType = Water)
  ...
  member _.``Wilderness scenario has varied terrain``() =
    ...
    let townId = %Guid.NewGuid()
    let wilderness = ScenarioDefinitions.createWildernessScenario wildernessId townId
    ...
    let blockedCount = terrainArray |> Array.filter (fun obj -> obj.TerrainType = Blocked) |> Array.length
    let waterCount = terrainArray |> Array.filter (fun obj -> obj.TerrainType = Water) |> Array.length
    let hazardCount = terrainArray |> Array.filter (fun obj -> obj.TerrainType = Hazard) |> Array.length
  ...
  member _.``Dungeon scenario has appropriate features``() =
    ...
    let wildernessId = %Guid.NewGuid()
    let dungeon = ScenarioDefinitions.createDungeonScenario dungeonId wildernessId
