...
type ``Pathfinding Tests``() =
  ...
  member _.``AStar finds path around obstacles``() =
    ...
    | ValueSome path ->
      ...
      let startDistance = sqrt((path.[0].X - start.X) ** 2f + (path.[0].Y - start.Y) ** 2f)
      let endDistance = sqrt((path.[path.Length - 1].X - goal.X) ** 2f + (path.[path.Length - 1].Y - goal.Y) ** 2f)
      Assert.True(startDistance < 32f, $"Start distance too far: {startDistance}")
      Assert.True(endDistance < 32f, $"End distance too far: {endDistance}")
    | ValueNone ->
      Assert.True(false, "Expected to find a path")
