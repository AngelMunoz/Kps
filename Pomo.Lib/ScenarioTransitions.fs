...
type TransitionState =
  ...
  | InProgress of targetScenario: Guid<ScenarioId> * targetPosition: Position * progress: float32
  | Completed of targetScenario: Guid<ScenarioId> * targetPosition: Position

module TransitionDetection =
  let checkProximity (entityPos: Position) (transitions: ScenarioTransition[]) : TransitionTrigger voption =
  ...
  let detectTransitions 
    ...
    (scenario: Scenario) 
    : aval<HashMap<Guid<EntityId>, TransitionTrigger>> = adaptive {
...
module TransitionExecution =
  ...
  let migrateEntity 
    ...
    (toScenarioId: Guid<ScenarioId>)
    : (Guid<ScenarioId> * EntityComponents) * (Guid<ScenarioId> * Guid<EntityId>) =
    
    let preservedEntity = preserveEntityState entity
    let updatedEntity = { preservedEntity with Position = newPosition }
  ...
  let executeTransition
    ...
    | Some currentScenario ->
      ...
      | Some entity ->
        let ((targetScenarioId, updatedEntity), (sourceScenarioId, entityToRemove)) =
          migrateEntity entityId entity trigger.ToPosition currentScenarioId trigger.ToScenarioId
        ...
        transact (fun () ->
          activeScenarioId.Value <- targetScenarioId
        )
...
module VisualTransitionEffects =
  ...
  let updateEffect (deltaTime: float32) (effect: TransitionEffect) : TransitionEffect =
...
module ScenarioDefinitions =
  let createTownScenario (id: Guid<ScenarioId>) : Scenario = {
    ...
    BattleEnabled = false
    CombatType = PvE
    TerrainObjects = IndexList.ofList [
  ...
  let createWildernessScenario (id: Guid<ScenarioId>) (townId: Guid<ScenarioId>) : Scenario = {
    Id = id
  ...
  let createDungeonScenario (id: Guid<ScenarioId>) (wildernessId: Guid<ScenarioId>) : Scenario = {
    Id = id
