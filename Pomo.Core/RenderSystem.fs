...
module RenderSystem =
  ...
  let init (gd: GraphicsDevice) =
    if isNull smallCircle then smallCircle <- makeCircle gd 12
    if isNull mediumCircle then mediumCircle <- makeCircle gd 16
    if isNull largeCircle then largeCircle <- makeCircle gd 20

  let private circleForStage stage =
    match stage with
    | Stage.First -> smallCircle
  ...
  let draw (sb: SpriteBatch) (pixel: Texture2D) (hud: SpriteFont voption) (state: GameState) (view: Matrix) =
    ...
    for struct (id, comp) in entities do
      ...
      let color =
        if comp.Factions |> HashSet.contains Faction.Player then Color.Green
        elif comp.Factions |> HashSet.contains Faction.Enemy then Color.Red
        elif comp.Resources.Status = Status.Dead then Color.Gray
        else Color.Blue
      ...
      let fillRect = Rectangle(int barX, int barY, int (barW * hpRatio), int barH)
      sb.Draw(pixel, backRect, Color(60,60,60))
      sb.Draw(pixel, fillRect, Color.LimeGreen)
      match hud with
      | ValueSome font ->
        let label = string comp.Identity.Family + "/" + string comp.Identity.Stage
